...
async def main():
    ...
    # Đăng ký cả 2 tool để LLM có thể chọn gọi
    ...
    # Dùng while thay vì if: model agentic (GLM-4.7) có thể gọi tool nhiều vòng
    # liên tiếp trước khi đưa ra câu trả lời cuối — lặp tới khi không còn tool_calls.
    while response.tool_calls:
        ...
        # Gọi lại LLM với kết quả tool; nếu còn tool_calls thì vòng while lặp tiếp
        response = await llm_with_tools.ainvoke(messages)
        messages.append(response)
